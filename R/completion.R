...
workspace_completion <- function(workspace, token, package = NULL, exported_only = TRUE) {
    ...
    if (is.null(package) || exported_only) {
        for (nsname in packages) {
            ...
            functs <- ns$get_completions(funct_only = TRUE, exported_only = TRUE)
            ...
            nonfuncts <- ns$get_completions(funct_only = FALSE, exported_only = TRUE)
        ...
        if (!is.null(ns)) {
            tag <- paste0("{", package, "}")
            functs <- ns$get_completions(funct_only = TRUE, exported_only = FALSE)
            functs <- functs[startsWith(functs, token)]
            functs_completions <- lapply(functs, function(object) {
                list(label = object,
                     kind = CompletionItemKind$Function,
            ...
            nonfuncts <- ns$get_completions(funct_only = FALSE, exported_only = FALSE)
            nonfuncts <- nonfuncts[startsWith(nonfuncts, token)]
            nonfuncts_completions <- lapply(nonfuncts, function(object) {
                list(label = object,
                     kind = CompletionItemKind$Field,
